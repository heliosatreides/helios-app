      ...
      <div className="space-y-1">
        {[
          ...
          <div key={lb} className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground/80">{lb}</span>
            <button onClick={() => copy(val)} className="text-xs text-muted-foreground hover:text-amber-400 font-mono">
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-background border border-border p-5 space-y-4">
            ...
            <div className="border-t border-border pt-4 flex items-center gap-3 flex-wrap">
              <input value={mood} onChange={e => setMood(e.target.value)} placeholder="Enter a mood/theme (e.g., 'ocean', 'sunset', 'forest')" className="flex-1 bg-secondary border border-border px-3 py-2 text-foreground text-sm outline-none focus:border-amber-500 min-w-48" />
              <button onClick={paletteFromMood} disabled={loading || !mood.trim()} className="px-4 py-2 bg-purple-600 text-white text-sm font-semibold hover:bg-purple-500 disabled:opacity-50">✨ From Mood</button>
            </div>
            {moodColors && (
              <div className="space-y-2">
                <div className="text-xs text-muted-foreground">AI Suggested Colors:</div>
                <div className="flex gap-2 flex-wrap">
                  {moodColors.split(',').map(c => c.trim()).filter(c => c.startsWith('#')).map(c => (
                    <div key={c} className="flex flex-col items-center gap-1">
                      <div className="w-12 h-12 border border-border cursor-pointer hover:border-amber-500" style={{ backgroundColor: c }} onClick={() => { setHex(c); generate_palette(); }} />
          ...
          {palette && (
            <div className="space-y-6">
              ...
              <div className="bg-background border border-border p-4 space-y-2">
                <h3 className="text-sm font-semibold text-muted-foreground">CSS Custom Properties</h3>
                <pre className="text-xs text-muted-foreground font-mono overflow-auto">{cssVars}</pre>
              </div>

              {/* AI name + Save */}
              <div className="bg-background border border-border p-4 space-y-3">
                <h3 className="text-sm font-semibold text-muted-foreground">✨ Name & Save</h3>
                <div className="flex gap-2">
                  <button onClick={namePalette} disabled={loading} className="px-3 py-2 bg-purple-600 text-white text-sm hover:bg-purple-500 disabled:opacity-50">✨ Suggest Name</button>
      ...
      {tab === 'saved' && (
        <div className="space-y-4">
          {palettes.length === 0 ? (
            <div className="text-center py-16 text-muted-foreground/80">No saved palettes. Generate and save one!</div>
          ) : (
            palettes.map(p => (
              <div key={p.id} className="bg-background border border-border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-foreground">{p.name}</h3>
                  <button onClick={() => setPalettes(prev => prev.filter(x => x.id !== p.id))} className="text-muted-foreground/80 hover:text-red-400">✕</button>
                </div>
                <div className="flex gap-2 flex-wrap">
                  {[p.palette.base, ...p.palette.complementary, ...p.palette.analogous, ...p.palette.triadic].map((c, i) => (
                    <div key={i} className="w-10 h-10 border border-border" style={{ backgroundColor: c.hex }} title={c.hex} />
                  ))}
                </div>
              </div>
            ))
