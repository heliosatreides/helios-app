...
function AddWatchForm({ onAdd, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-background border border-border p-5 space-y-4">
      <h3 className="text-foreground font-semibold">Add to Watchlist</h3>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-muted-foreground text-xs mb-1 block">Target Buy Price</label>
          <input className={inputCls} type="number" step="any" min="0" placeholder="500.00" value={form.targetPrice} onChange={set('targetPrice')} />
        </div>
        <div>
          <label className="text-muted-foreground text-xs mb-1 block">Notes</label>
    ...
    <div className="bg-background border border-border p-4 flex items-start justify-between gap-3">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {item.name && <span className="text-muted-foreground text-sm">{item.name}</span>}
        ...
        {item.targetPrice && (
          <p className="text-muted-foreground text-xs">Target: ${Number(item.targetPrice).toFixed(2)}</p>
        )}
        {item.notes && <p className="text-muted-foreground text-xs mt-1 truncate">{item.notes}</p>}
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          onClick={() => toggleStatus(item.id)}
          className="text-xs border border-border text-muted-foreground hover:text-foreground px-2 py-1 transition-colors"
