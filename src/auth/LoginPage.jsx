  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="border border-border p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input className={inputCls} type="password" placeholder="Enter password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete={mode === 'register' ? 'new-password' : 'current-password'} required />
            </div>
            {mode === 'register' && (
              <div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-foreground text-background py-2 text-sm font-medium hover:bg-foreground/90 disabled:opacity-50 transition-colors">
        ...
        <p className="text-center text-muted-foreground/60 text-xs mt-4">All data stored locally on your device</p>
      </div>
    </div>
  );
}
